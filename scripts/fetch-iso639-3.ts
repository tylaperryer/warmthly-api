 ...
 * - Unicode CLDR: For native names and locale data
 */
...
interface ISO6393Entry {
  Id: string; // ISO 639-3 code
  Part2B?: string; // ISO 639-2/B code
  Part2T?: string; // ISO 639-2/T code
  Part1?: string; // ISO 639-1 code
  Scope: string; // I(ndividual), M(acrolanguage), S(pecial)
  Type: string; // A(ncient), C(onstructed), E(xtinct), H(istorical), L(iving), S(pecial)
  Ref_Name: string; // Reference name (English)
  Comment?: string;
}
...
// Language family mappings (simplified - can be expanded)
const FAMILY_MAPPINGS: Record<string, string> = {
 ...
 */
async function downloadISO6393Table(): Promise<string> {
 ...
 */
function parseISO6393Table(tsvData: string): ISO6393Entry[] {
  const lines = tsvData.split('\n').filter(line => line.trim());
  const headers = lines[0].split('\t');
  
  const entries: ISO6393Entry[] = [];
  
  for (let i = 1; i < lines.length; i++) {
    const values = lines[i].split('\t');
    if (values.length < headers.length) continue;
    
    const entry: ISO6393Entry = {
      Id: values[0] || '',
    ...
    // This filters out macrolanguages, extinct languages, etc.
 ...
 * In production, this could fetch from Unicode CLDR or other sources
 */
function getNativeName(code: string, englishName: string, iso6391?: string): string {
 ...
 */
function generateLocale(code: string, iso6391?: string): string {
  const langCode = iso6391 || code;
  // Simple locale generation - can be enhanced with region data
 ...
 */
function getScript(code: string, iso6391?: string): string | undefined {
 ...
 */
function getFamily(code: string, iso6391?: string): string | undefined {
  const key = iso6391 || code;
 ...
 */
function isRTL(code: string, iso6391?: string): boolean {
  const key = iso6391 || code;
 ...
 */
function convertToLanguageEntries(iso6393Entries: ISO6393Entry[]): LanguageEntry[] {
 ...
 */
function generateTypeScriptCode(entries: LanguageEntry[]): string {
  const lines: string[] = [];
  
  // Group by family for better organization
  const byFamily = new Map<string, LanguageEntry[]>();
  const noFamily: LanguageEntry[] = [];
 ...
 */
function generateEntryLine(entry: LanguageEntry): string {
  const parts: string[] = [];
